...
Developer A is proud of this structure. ID first (primary key!), timestamps last (metadata!), business fields in the middle. There's a logic. A logic that lives entirely inside Developer A's head. Developer A will explain it at length in the code review. Nobody asked.
...
Your original logic is dead. The structure is now geological: layers of sediment with no guiding principle. Finding anything requires scanning the whole thing top to bottom, every time. You built a list you have to read like a novel. Nobody asked for a novel. Nobody wants to read your novel.
