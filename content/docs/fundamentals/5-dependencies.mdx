...
If you can't tell what a class depends on by looking at its constructor, your architecture is lying to you. And every lie it tells becomes a bug you'll find at runtime instead of compile time.
...
Two tests running in parallel both mutate the same singleton. Race condition. Flaky tests. The test passes locally, fails in CI, passes again when you re-run it. You spend an hour writing "works on my machine" in Slack. This is the singleton tax.
...
Change the singleton's interface? Now you're grep-ing the entire codebase hoping you got them all. The compiler can't help because the dependency is invisible. Invisible dependencies are the cockroaches of software architecture: everywhere, impossible to count, surviving every refactor.
...
container.register("LOGER", Logger); // ← Typo! Won't know until runtime
...
This keeps your domain logic centralized and your infrastructure dumb. The `Store` is a mechanical layer: it reads and writes. It doesn't know what makes a `User` valid any more than a filing cabinet knows what makes a contract legal. Business rules live in business objects.
