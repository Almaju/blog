...
With DI frameworks, you can't see what depends on what by reading the code. You configure strings in one file, inject by annotation in another, and hope the container resolves it correctly at runtime. When it doesn't, you get a message that says something failed to wire, no indication of where, and a Friday afternoon disappears. You find the bug: you typed `"DATABSE"`. The framework accepted it silently. Of course it did.
