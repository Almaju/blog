...
A comment that explains what code does is not documentation. It's a confession. The code had something to say and couldn't say it. You wrote a second layer of meaning on top of the first. Now you're maintaining two representations of the same thing, and they will drift. They always drift.
...
Code that's wrong breaks. A comment that's wrong just misleads, quietly, until something explodes in production and you spend three hours reading the wrong code path because the comment pointed you there.
...
Your function accepts `User` when it should accept `Admin`. The comment is documenting an unchecked precondition. Right now it's documented with text that nobody reads. Make it a type error instead.

**"// step 1: validate, step 2: transform, step 3: persist"**

You have three functions trapped inside one. Extract them. The comment is an informal table of contents for code that should have been split.

The pattern is always the same: the comment describes something the type system, the function structure, or the naming could express directly. The comment is a workaround for code that hasn't been finished yet.
...
**Rename the thing.** If you need a comment to explain what a variable or function does, the name is wrong. The name is the documentation.
...
What's not useful: `// TODO: refactor this`. Refactor what? Why? When? A TODO without context is noise with a timestamp. It will sit there for three years, mocking every developer who reads it, and nobody will ever act on it because nobody knows what "this" is.
...
Complex code is the problem. If the code needs a guide to be navigated, simplify the code. Comments help junior developers survive complex code; they don't help them understand it. And they definitely don't help them change it safely. You're not doing juniors a favor by documenting a mess. You're preserving the mess.
