...
Yes, by about 15%. But that verbosity is **honest**. Every error case is visible. Every recovery strategy is explicit. You can't accidentally ignore a failure. The verbosity is the point. Compare that 15% overhead to the time you spent debugging a silent exception that swallowed your error and returned `undefined`.
