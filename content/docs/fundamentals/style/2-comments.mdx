...
<Tabs items={['TypeScript', 'Rust', 'Python']}>
  ...
  <Tab value="Python">
  ```python
  def get_non_deleted_users(db: Database) -> list[User]:
      return db.query("SELECT * FROM users WHERE deleted_at IS NULL")
  ```
  </Tab>
</Tabs>
...
<Tabs items={['TypeScript', 'Rust', 'Python']}>
  ...
  def process(order: Order) -> None:
      # Can only process if payment is confirmed and stock is reserved
      if order.payment_status == "confirmed" and order.stock_status == "reserved":
          order.status = "processing"
          fulfillment.dispatch(order)
  ...
  class ReadyOrder:
      ...
      stock_status: Literal["reserved"]


  def dispatch_to_fulfillment(order: ReadyOrder) -> None:
      order.status = "processing"
      fulfillment.dispatch(order)
  ```
  </Tab>
