...
@dataclass
class User:
    id: str              # Primary key goes first, obviously
    email: str
    name: str
    role: UserRole
    created_at: datetime # Metadata at the end
...
class User:
    ...
    updated_at: datetime
    last_login_at: datetime | None = None
    phone_number: str | None = None  # Where does this go?
```
</Tab>
</Tabs>
...
```typescript
/**
 * Method ordering: constructor, then public (alpha), then private (alpha).
 */
...
<Tab value="Python">
```python
# Method ordering: __init__, then public (alpha), then private (alpha).
class UserRepository:
    ...
    async def find_user_by_id(self, id: str) -> User | None: ...

    async def _execute_query(self, sql: str) -> QueryResult: ...
    def _validate_user_data(self, data: UserData) -> None: ...
```
</Tab>
</Tabs>
...
# pyproject.toml
[tool.ruff]
select = ["I"]  # isort: sorts and groups imports automatically
...
<Tab value="TypeScript">
```typescript
// Bad: proximity implies relationship
interface User {
  firstName: string;
...
// Good: types make the relationship explicit
interface UserName { first: string; last: string; }
interface UserContact { email: string; phone: string; }
interface User {
...
class User:
    first_name: str
    last_name: str  # "These go together!"
    email: str
    phone: str      # "These go together!"
