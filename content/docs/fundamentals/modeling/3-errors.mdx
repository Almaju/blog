...
function getUserById(id: string): User {
  // Could throw NetworkError, NotFoundError, ValidationError...
...
fn get_user_by_id(id: &str) -> Result<User, UserError> {
    // The type system makes errors first-class — you cannot ignore them
}
```
</Tab>
<Tab value="Python">
...
def get_user_by_id(id: str) -> "User":
    # Could raise NetworkError, NotFoundError...
    # Type signature reveals none of this
    ...
...
UserError = NetworkError | NotFoundError

def get_user_by_id(id: str) -> Result["User", UserError]:
    # Now the signature tells the truth
    ...
```
</Tab>
...
try:
    ...
    try:
        ...
        try:
            comments = get_comments_by_posts(posts)
            return process_comments(comments)
        except Exception:
            pass  # what do I even do here?
    except Exception:
        pass
except Exception:
    pass

# Result Heaven — every failure is visible
def get_comments_for_user(id: str) -> Result[list[Comment], AppError]:
    user_result = get_user_by_id(id)
    if isinstance(user_result, Err):
        return user_result
    posts_result = get_posts_by_user(user_result.value)
    if isinstance(posts_result, Err):
        return posts_result
    comments_result = get_comments_by_posts(posts_result.value)
    if isinstance(comments_result, Err):
        return comments_result
    return Ok(process_comments(comments_result.value))
```
</Tab>
</Tabs>
...
# Pattern match on the error. Make a real decision.
match err:
    case NetworkError(delay=delay):
        await asyncio.sleep(delay * (2 ** attempt))
    case AuthError(refresh_token=token):
        await refresh_auth(token)
    case RateLimitError(retry_after=retry_after):
        await asyncio.sleep(retry_after)
    case ValidationError():
        return Err(err)  # Not retryable — user needs to fix their input.
```
</Tab>
</Tabs>
...
<Tab value="TypeScript">
```typescript
type UserError   = { category: "user";     display: string; recoverable: boolean };
type SystemError = { category: "system";   alert: string;   context: Record<string, unknown> };
type ExternalError = { category: "external"; service: string; retryable: boolean; retryAfter?: number };
...
<Tab value="Rust">
```rust
enum AppError { User(UserError), System(SystemError), External(ExternalError) }

fn handle_error(error: AppError) {
    match error {
        AppError::User(e) => {
            show_toast(&e.display, ToastLevel::Error);
            if e.recoverable { enable_retry(); }
        }
        AppError::System(e) => {
        ...
        AppError::External(e) => {
            if e.retryable { schedule_retry(&e.service, e.retry_after); }
            else { show_service_unavailable(&e.service); }
        }
    }
}
```
...
AppError = UserError | SystemError | ExternalError

def handle_error(error: AppError) -> None:
    match error:
        case UserError(display=msg, recoverable=recoverable):
            show_toast(msg, "error")
            if recoverable: enable_retry()
        case SystemError(alert=alert, context=ctx):
            show_toast("Something went wrong", "error")
            alert_engineering(alert, ctx)
        case ExternalError(service=svc, retryable=True, retry_after=after):
            schedule_retry(svc, after)
        case ExternalError(service=svc, retryable=False):
            show_service_unavailable(svc)
```
</Tab>
</Tabs>
...
<Tab value="TypeScript">
```typescript
// OK to throw — program can't run without these
const config = loadConfig();
const db = await connectDatabase(config.dbUrl);

// NOT OK to throw — return Result instead
function getUser(id: string): Result<User, NotFoundError> {
  const user = db.query("SELECT * FROM users WHERE id = ?", id);
  if (!user) return { type: "Err", err: { type: "NotFoundError", userId: id } };
...
<Tab value="Rust">
```rust
// OK to panic — program can't run without these
...
// NOT OK to panic — return Result instead
...
<Tab value="Python">
```python
# OK to raise — program can't run without these
config = load_config()
db = connect_database(config.db_url)

# NOT OK to raise — return Result instead
def get_user(id: str) -> Result["User", NotFoundError]:
    user = db.query("SELECT * FROM users WHERE id = ?", id)
    if user is None:
        return Err(NotFoundError(user_id=id))
    return Ok(user)
```
</Tab>
</Tabs>
...
<Tab value="TypeScript">
```typescript
// Wrap throwing libraries at the door — one try-catch, typed result out
function safeFetch<T>(url: string): Result<T, NetworkError> {
  try {
    const data = await fetch(url).then(r => r.json());
    return { type: "Ok", data };
  } catch (e) {
    return { type: "Err", err: { type: "NetworkError", message: String(e) } };
  }
}
```
</Tab>
...
<Tab value="Python">
```python
# Wrap throwing libraries at the door — one try/except, typed result out
def safe_fetch(url: str) -> Result[dict, NetworkError]:
    try:
        response = httpx.get(url)
        response.raise_for_status()
        return Ok(response.json())
    except Exception as e:
        return Err(NetworkError(message=str(e)))
```
</Tab>
</Tabs>
