...
You already know this works. You use `Vec` every day without thinking about pointer arithmetic or reallocation strategies. You use `HashMap` without once caring about the collision resolution function. Nobody reimplements these. That's the point. The whole point. Your job is to build the same thing for your domain.
...
class Customer extends Human {
  constructor(name: string, dateOfBirth: Date, public customerId: CustomerId) {
    super(name, dateOfBirth);
  }
}

class Employee extends Human {
  constructor(name: string, dateOfBirth: Date, public employeeId: EmployeeId) {
    super(name, dateOfBirth);
  }
}

// What if someone is both? What if Human needs to change?
// These questions don't have good answers.

// ✅ Composition. Each piece is independent.
type PersonalInfo = {
  name: string;
  dateOfBirth: Date;
};
...
<Tab value="Rust">
```rust
// Rust doesn't have inheritance. This is not an accident.
// The language made the decision for you.
