...
// Six months later, UserService gets this:
class UserService {
  async banUser(userId: string): Promise<void> {
    // Slightly different. Also works. Now you have two.
    await this.db.query(
      "UPDATE users SET banned_at = ?, is_active = false WHERE id = ?",
      new Date(), userId
    );
  }
}
...
// The feature is: user.ban().save(store)
// One place to look. One way to do it.
```
</Tab>
<Tab value="Rust">
```rust
...
impl UserRepository {
    async fn save(&self, user: &User) -> Result<(), Error> {
        self.db.query("INSERT INTO users ...", user).await
    }
