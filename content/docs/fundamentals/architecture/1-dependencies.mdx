...
#[tokio::test]
async fn test_creates_user() {
    let service = UserService::new(TestDatabase::new());
...
def test_creates_user_isolated():
    service = UserService(MemDatabase())
...
async def main() -> None:
    ...
    payment_service = PaymentService(db, config.stripe_key)

    app = App(user_service, order_service, payment_service)
    await app.listen(config.port)
