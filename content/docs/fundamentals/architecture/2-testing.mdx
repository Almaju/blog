...
<Tab value="TypeScript">
```typescript
// Untestable: clock baked in
class SessionService {
  createSession(userId: string): Session {
    return {
      userId,
      createdAt: new Date(),         // What time is "now" in a test?
      expiresAt: new Date(Date.now() + 3600_000),
    };
  }
}

// Testable: clock injected
interface Clock {
  now(): Date;
}
...
fn session_expires_one_hour_after_creation() {
    ...
    let service = SessionService { clock };
    let session = service.create_session("user-1");
