...
In 1999, NASA lost a $327 million Mars orbiter because one engineering team measured thrust in pound-force·seconds and another in newton-seconds. Both were `float`. The type system had nothing to say about it.
...
You know `price` must be positive. You wrote a comment saying so. Three months later someone passes `-50` and you get a runtime error in production. Comments are not enforcement. Newtypes are:
