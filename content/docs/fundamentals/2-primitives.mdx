...
We evolved past programming in raw bytes because bytes are too primitive to think in. The same logic applies one level up. Finish the job:
...
In 1999, NASA lost a $327 million Mars orbiter because one engineering team measured thrust in pound-force·seconds and another in newton-seconds. Both were `float`. The type system had nothing to say about it. The orbiter burned up in the atmosphere. Your bug will be smaller, but the mechanism is identical.
...
You know `price` must be positive. You wrote a comment saying so. Three months later someone passes `-50` and you get a runtime crash in production, at 3am, because comments are not enforcement. Comments are suggestions that the next developer will ignore. Newtypes are not:
