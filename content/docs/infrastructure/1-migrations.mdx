...
**Q: What if two developers change the schema at the same time?**

A: Same as merge conflicts in code. Both `schema.sql` and `migrate.sql` will conflict. Merge the schema changes, rewrite `migrate.sql` to handle both transformations as one operation. It's actually easier than two numbered migration files that both increment the same version counter.
...
**Q: Doesn't `migrate.sql` become stale after deployment?**

A: Yes, and that's fine. Once applied everywhere, `migrate.sql` is just documentation until the next schema change overwrites it. The full migration history is in git for anyone who needs it.
