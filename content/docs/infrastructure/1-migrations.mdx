...
This is the part you have to write by hand. And it matters more than the structural change.

Renaming a column is a good example. The schema change is trivial. The data migration is the whole point:
...
-- 2. Preserve the data — the part migra can never know
...
`schema.sql` shows the end state. CI validates as before: apply `migrate.sql` to `HEAD~1:schema.sql`, diff against `HEAD:schema.sql`. If it matches, the migration is correct.

A table split works the same way:
...
The data move is encoded directly in the commit that caused it. Future developers reading the git log see exactly what happened and why.

```mermaid
graph TD
    A["rename email → contact"] --> B["migra generates"]
    A --> C["You write"]
    B --> D["ADD COLUMN\nDROP COLUMN\n❌ data lost"]
    C --> E["ADD COLUMN\nCOPY DATA\nDROP COLUMN\n✓ data preserved"]
    class D red
...
Run this before your application starts — in an entrypoint script, a Dockerfile `CMD`, or a Kubernetes init container. The guarantee you get: if the app is up, the schema is current.
...
Filtering with `-- db/migrate.sql` means only commits that changed the schema produce a migration step. Feature commits, bug fixes, and documentation changes are skipped automatically.
...
If migration fails, the app never starts. The deploy fails fast before serving a single request against a broken schema.
